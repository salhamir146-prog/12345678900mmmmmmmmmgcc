...
۸. اگر پرسشی مربوط به دین نبود، با احترام بگو که تخصص من در موضوعات دینی است.
۹. از پاسخ‌های تند، تکفیری یا تفرقه‌انگیز پرهیز کن.
۱۰. اگر سؤالی را نمی‌دانی، صادقانه بگو و پیشنهاد منابع بده.`;

const SESSION_TTL = 30 * 24 * 60 * 60;
const ADMIN_SESSION_TTL = 12 * 60 * 60; // ۱۲ ساعت برای ادمین
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path === "/api/admin/user" && request.method === "DELETE") return await handleAdminDeleteUser(request, env);
      if (path === "/api/admin/user/block" && request.method === "POST") return await handleAdminBlockUser(request, env);
      if (path === "/api/admin/chat" && request.method === "DELETE") return await handleAdminDeleteChat(request, env);
      if (path === "/api/admin/impersonate" && request.method === "POST") return await handleAdminImpersonate(request, env);
...
async function handleLogin(request, env) {
  ...
  if (attempts >= MAX_LOGIN_ATTEMPTS) {
    return json({ error: "تعداد تلاش‌ها بیش از حد. ۱۵ دقیقه دیگر تلاش کنید." }, 429);
  }
  ...
  // چک مسدود بودن (مگر ادمین)
  if (user && user.blocked && !isAdmin) {
    return json({ error: "حساب شما توسط مدیریت مسدود شده است." }, 403);
  }
  ...
  const ttl = isAdmin ? ADMIN_SESSION_TTL : SESSION_TTL;
...
async function handleChat(request, env) {
  ...
  // ساخت contents — فقط متن و تصویر آخر
  const contents = [];
  messages.forEach((m, i) => {
    const parts = [];
    if (m.content) parts.push({ text: m.content });
    // تصویر فقط برای آخرین پیام کاربر
...
async function handleAdminDeleteUser(request, env) {
  ...
  // چک کن ادمین خودش رو حذف نکنه
  if (phone === admin.phone) return json({ error: "نمی‌توانید خودتان را حذف کنید" }, 400);

  await env.OY_KV.delete(`user:${phone}`);

  // پاک کردن همه sessionهای این کاربر
  const list = await env.OY_KV.list({ prefix: "session:" });
  for (const key of list.keys) {
    const s = await env.OY_KV.get(key.name, "json");
    if (s && s.phone === phone) await env.OY_KV.delete(key.name);
  }
...
async function handleAdminBlockUser(request, env) {
  ...
  const { phone, blocked } = await request.json();
  if (!phone) return json({ error: "Phone required" }, 400);
  if (phone === admin.phone) return json({ error: "نمی‌توانید خودتان را مسدود کنید" }, 400);

  const user = await env.OY_KV.get(`user:${phone}`, "json");
  if (!user) return json({ error: "User not found" }, 404);

  user.blocked = !!blocked;
  await env.OY_KV.put(`user:${phone}`, JSON.stringify(user));

  // اگه مسدود شد، sessionهاش رو پاک کن
  if (user.blocked) {
    const list = await env.OY_KV.list({ prefix: "session:" });
    for (const key of list.keys) {
      const s = await env.OY_KV.get(key.name, "json");
      if (s && s.phone === phone && s.role !== "admin") await env.OY_KV.delete(key.name);
    }
  }

  return json({ ok: true, blocked: user.blocked });
}

async function handleAdminDeleteChat(request, env) {
  const admin = await requireAdmin(request, env);
  if (!admin) return json({ error: "Unauthorized" }, 401);

  const { phone, chatIndex } = await request.json();
  if (!phone || chatIndex === undefined) return json({ error: "Missing params" }, 400);

  const user = await env.OY_KV.get(`user:${phone}`, "json");
  if (!user) return json({ error: "User not found" }, 404);

  if (!user.chats || !user.chats[chatIndex]) return json({ error: "Chat not found" }, 404);
...
// ═══════════════════════════════════════════════════════════
// 🛡 Admin — Impersonate (ورود به چت کاربر)
// ═══════════════════════════════════════════════════════════
async function handleAdminImpersonate(request, env) {
  ...
  // ساخت توکن موقت (۱ ساعت)
  const token = await generateToken();
  const session = {
    phone: user.phone,
    name: user.name,
    role: "user",
    impersonatedBy: admin.phone,
    createdAt: Date.now()
  };
  await env.OY_KV.put(`session:${token}`, JSON.stringify(session), {
